...
!  We employ Hermit splines and orthogonal collocations.
...
!    Xr=rightBound
    ...
    if(SplineType.eq.0) then
      ...
      stop 'SplineType can be only Cubic(0) or Quintic(1)'
    end if
...
!**************************************************************
      ...
      if (((i.eq.0).or.(i.eq.1)).and.(BCLeft.eq.2)) HSPlineBC=HSPline(Ncx+I1,X) + HSPline(I1,X)
      ...
      if (((i.eq.0).or.(i.eq.1)).and.(BCLeft.eq.2)) HSPlineBC1=HSPline1(Ncx+I1,X) + HSPline1(I1,X)
      ...
      if (((i.eq.0).or.(i.eq.1)).and.(BCLeft.eq.2)) HSPlineBC2=HSPline2(Ncx+I1,X) + HSPline2(I1,X)
...
!-------------------------------------------------------------------!
	...
	DR=X-XI
	IF(SplineType.eq.0) THEN
          RES=-2.0d0*DL*DL*(DR-0.5d0*HK)/(HK*HK*HK)
	ELSE IF (SplineType.eq.1) THEN
	...
	DR=X-XI1
	IF(SplineType.eq.0) THEN
          RES=2.0d0*(DL+0.5d0*HK)*DR*DR/(HK*HK*HK)
	ELSE IF (SplineType.eq.1) THEN
...
!
	...
	DR=X-XI
	IF(SplineType.eq.0) THEN
          res=-4.0d0*(DR-0.5d0*HK+2.0d0*DL)/(HK*HK*HK)
	ELSE IF (SplineType.eq.1) THEN
	...
	DR=X-XI1
	IF(SplineType.eq.0) THEN
          res=4.0d0*(DL+0.5d0*HK+2.0d0*DR)/(HK*HK*HK)
	ELSE IF (SplineType.eq.1) THEN
