...
class Component:
    ...
    def init_laplacian(self):
        ...
        self.mu_2 = 0.5 * (
            1 / (self.masses[beta] + self.masses[gamma]) + 1 / self.masses[alpha]
        )
        ...
        self.laplace_x = -self.mu_1 * sp.sparse.kron(
            self.ddx, self.ident_y, format=self.format
        )
        self.laplace_y = -self.mu_2 * sp.sparse.kron(
            self.ident_x, self.ddy, format=self.format
        )
    ...
    def decompose(self):
        ...
        print(f"index {self.idx}, reduced mass: m1={1/self.mu_1}, m2={1/self.mu_2}")
        ...
        self.Wr = lambda x: sp.sparse.kron(self.Wxl, self.Iy) @ (
            sp.sparse.kron(self.Ix, self.Wyl) @ x
        )
        self.Wl = lambda x: sp.sparse.kron(self.Wxr, self.Iy) @ (
            sp.sparse.kron(self.Ix, self.Wyr) @ x
        )

    def diag_inverse(self, E):
        self.diag = (
            sp.sparse.kron(self.xvals, self.yvals)
            + sp.sparse.kron(self.Ix, self.Iy)
            - E * sp.sparse.kron(self.Ix, self.yvals)
        ).toarray()
    ...
    def init_potential(self):
        ...
        vx = np.array([self.lv(xi, 0) for xi in self.spl_x.collocPoints])
        self.vx = sp.sparse.diags(vx, format=self.format)
        self.v = sp.sparse.diags(potential, format=self.format)
...
class MulticomponentRHS:
            ...
            for beta in range(3):
                if alpha != beta:
                    ...
                    s2 = (
                        ...
                        * sgn
                        * (
                            (total_mass - masses[alpha] - masses[beta])
                            * total_mass
                            / (total_mass - masses[alpha])
                            / (total_mass - masses[beta])
                        )
                    )
                else:
                    c1 = c2 = 1.0
    ...
    def convert_gridpoints(self, to: Component, frm: Component):
        orig_colloc_grids = np.array(
            [pair for pair in product(frm.spl_x.collocPoints, frm.spl_y.collocPoints)]
        )
        ...
        for idx, (xi, yj) in enumerate(zip(*new_colloc_grids)):
            ...
            for i in range(to.spl_x.getSpaceDim()):
                ...
                for j in range(to.spl_y.getSpaceDim()):
                    ...
                    rhs[row_idx, col_idx] = to.spl_x.fBSplineBC(
                        xi, i
                    ) * to.spl_y.fBSplineBC(yj, j)
...
class Faddeev1D3B:
    def __init__(self, grids, masses=None, regulizers=None, left_bc=1, right_bc=1):
        ...
        self.nMatr = sp.sparse.bmat(
            (
                [
                    [self.alpha.nMatr, None, None],
                    [None, self.beta.nMatr, None],
                    [None, None, self.gamma.nMatr],
                ]
            ),
            format=self.format,
        )
    ...
    def preconditioner(self, E):
        def mv(t):
            ...
            t[: self.alpha.dim] = self.alpha.Wl(
                self.alpha.diag_inverse(E) @ self.alpha.Wr(t_alpha)
            )
            beta_bound = self.alpha.dim + self.beta.dim
            t_beta = t[self.alpha.dim : beta_bound]
            t[self.alpha.dim : beta_bound] = self.beta.Wl(
                self.beta.diag_inverse(E) @ self.beta.Wr(t_beta)
            )
            t_gamma = t[beta_bound:]
            t[beta_bound:] = self.gamma.Wl(
                self.gamma.diag_inverse(E) @ self.gamma.Wr(t_gamma)
            )
            t = self.c @ t
            t = -self.v @ t
            return t
    ...
    def init_fhamiltonian(self):
        self.h0 = sp.sparse.bmat(
            (
                [
                    [self.alpha.laplace, None, None],
                    [None, self.beta.laplace, None],
                    [None, None, self.gamma.laplace],
                ]
            ),
            format=self.format,
        )
        self.ident = sp.sparse.bmat(
            (
                [
                    [self.alpha.ident, None, None],
                    [None, self.beta.ident, None],
                    [None, None, self.gamma.ident],
                ]
            ),
            format=self.format,
        )
        self.v = sp.sparse.bmat(
            (
                [
                    [self.alpha.v, None, None],
                    [None, self.beta.v, None],
                    [None, None, self.gamma.v],
                ]
            ),
            format=self.format,
        )
        rhs = self.rhs = MulticomponentRHS(self.masses)
        from_alpha = self.from_alpha = [
            None,
            rhs.transform(self.beta, self.alpha),
            rhs.transform(self.gamma, self.alpha),
        ]
        from_beta = self.from_beta = [
            rhs.transform(self.alpha, self.beta),
            None,
            rhs.transform(self.gamma, self.beta),
        ]
        from_gamma = self.from_gamma = [
            rhs.transform(self.alpha, self.gamma),
            rhs.transform(self.beta, self.gamma),
            None,
        ]
        ...
        for i in range(self.alpha.spl_x.getSpaceDim()):
            ...
            for j in range(self.alpha.spl_y.getSpaceDim()):
                if self.alpha.spl_y.fBSplineBC(ya, j) == 0.0:
                    continue
                values[
                    i * self.alpha.spl_y.getSpaceDim() + j
                ] = self.alpha.spl_x.fBSplineBC(xa, i) * self.alpha.spl_y.fBSplineBC(
                    ya, j
                )
        ...
        for i in range(self.beta.spl_x.getSpaceDim()):
            ...
            for j in range(self.beta.spl_y.getSpaceDim()):
                if self.beta.spl_y.fBSplineBC(yb, j) == 0.0:
                    continue
                values[
                    alpha_dim + i * self.beta.spl_y.getSpaceDim() + j
                ] = self.beta.spl_x.fBSplineBC(xb, i) * self.beta.spl_y.fBSplineBC(
                    yb, j
                )
        ...
        for i in range(self.gamma.spl_x.getSpaceDim()):
            ...
            for j in range(self.gamma.spl_y.getSpaceDim()):
                if self.gamma.spl_y.fBSplineBC(yg, j) == 0.0:
                    continue
                values[
                    alpha_dim + beta_dim + i * self.gamma.spl_y.getSpaceDim() + j
                ] = self.gamma.spl_x.fBSplineBC(xg, i) * self.gamma.spl_y.fBSplineBC(
                    yg, j
                )
...
class ReducedFaddeev1D3B:
    def __init__(self, grids, masses=None, regulizers=None, left_bc=1, right_bc=1):
        ...
        self.nMatr = sp.sparse.bmat(
            [[self.alpha.nMatr, None], [None, self.beta.nMatr]], format=self.format
        )

    def init_fhamiltonian(self):
        self.h0 = sp.sparse.bmat(
            [[self.alpha.laplace, None], [None, self.beta.laplace]], format=self.format
        )
        self.ident = sp.sparse.bmat(
            [[self.alpha.ident, None], [None, self.beta.ident]], format=self.format
        )
        self.v = sp.sparse.bmat(
            [[self.alpha.v, None], [None, self.beta.v]], format=self.format
        )
        rhs = self.rhs = MulticomponentRHS(self.masses)
        ...
        v3_ga = [
            self.gamma.lv(x, y)
            for x, y in zip(*rhs.convert_gridpoints(self.gamma, self.alpha))
        ]
        self.v3_ga = sp.sparse.diags(v3_ga, format=self.format)

        v3_gb = [
            self.gamma.lv(x, y)
            for x, y in zip(*rhs.convert_gridpoints(self.gamma, self.beta))
        ]
        self.v3_gb = sp.sparse.diags(v3_gb, format=self.format)
        self.rhs_v = sp.sparse.bmat([[self.v3_ga, None], [None, self.v3_gb]], format=self.format)
