...
SparseRMatrix Hamiltonian3D::generateTheHamiltonian()
{
    ...
    diagv = SparseCMatrix(nMax, nMax);
    vxy = SparseRMatrix(nMax, nMax);
    vxy.reserve(Eigen::VectorXi::Constant(nMax, 20));
    diagv.reserve(Eigen::VectorXi::Constant(nMax, 1));
    ...
    for (int i = 0; i < nMax; i++){
        ...
        diagv.insert(i, i) = 0.5*potential;
    }
    ham = vxy + laplacian;
...
CVector Hamiltonian3D::evolutionStep(CVector state, int iter)
{
    ...
    */
    ...
    ColSpCMat St = eye + 0.5 * im * diagv;
    ColSpCMat Ft = eye - 0.5 * im * diagv;

    // std::cout << "St " << St << std::endl;
    // std::cout << "Ft " << Ft << std::endl;
    Eigen::SparseLU<ColSpCMat> solverPt;
    Eigen::SimplicialLDLT<ColSpCMat> solverVt;
    ColSpCMat p = pMatr.cast<std::complex<double>>();
    solverPt.compute(p);
    solverVt.compute(Ft);
    std::cout << "check 1" << std::endl;
    std::cout << "diagv " << diagv << std::endl;
